...
function buildBannerForm($type, $aBanner, &$oComponent=null, $formDisabled=false)
{
    ...
    if (OA_Permission::isAccount(OA_ACCOUNT_ADMIN) || OA_Permission::isAccount(OA_ACCOUNT_MANAGER)) {
        ...
        $form->addElement('text', 'keyword', $GLOBALS['strKeyword']);
         $form->addRule('keyword', "require", 'required');
         $form->addRule('keyword', 'only character', 'regex', '/^[A-z ]+$/');
         
        //  $form->addElement('html', 'video_status_info1', '<span style="font-size:100%;"><a href="">MyLink</a></span>' );
        //$form->addElement('text', 'gender', $GLOBALS['strGender']);
       // $form->addElement('radio', 'gender', $GLOBALS['strGender'], 'Male', 'male'); // add by ram
      //  $form->addElement('radio', 'gender', '        ', 'Female', 'female');
        $genderarray['radio1']= $form->createElement('radio', 'gender', null, 'Male', 'male'); // add by ram
        $genderarray['radio2']= $form->createElement('radio', 'gender', null, 'Female', 'female');
        
        $form->addGroup($genderarray, $GLOBALS['strGender'].'_group', $GLOBALS['strGender'], "<br>", false);
        
        //$form->addelement($genderarray,$GLOBALS['strGender']);
        
        //$form->addElement('text', 'minage', $GLOBALS['strAgeMin']);
        $minAgeElem = $form->createElement('text', 'minage', $GLOBALS['strAgeMin']);
        $minAgeElem->setAttribute('maxlength', '2');
        $minAgeElem->setAttribute("style", "width:30px");
       
        $form->addElement($minAgeElem);
        
        $maxAgeElem = $form->createElement('text', 'maxage', $GLOBALS['strAgeMax']);
        $maxAgeElem->setAttribute('maxlength', '2');
        $maxAgeElem->setAttribute("style", "width:30px");
        $form->addElement($maxAgeElem);
        
        $form->addRule('minage', "require", 'required');
        $form->addRule('minage', "number only", 'numeric');
        
        $form->addRule('maxage', "require", 'required');
        $form->addRule('maxage', "number only", 'numeric');
        //$form->addElement('text', 'maxage', $GLOBALS['strAgeMax']);
...
function processForm($bannerid, $form, &$oComponent, $formDisabled=false)
{
    ...
    $aVariables['gender']        = $aFields['gender']; //add by ram    
    $aVariables['minage']        = $aFields['minage']; //add by ram  
    $aVariables['maxage']        = $aFields['maxage']; //add by ram  
    $editSwf = false;
